// ! This file is copied from actix-web, and modified to fit the needs of this project.
...
impl ContentDisposition {
    ...
    pub fn from_raw(hv: &headers::HeaderValue) -> std::io::Result<Self> {
        ...
        while !left.is_empty() {
            ...
            if let Some(param_name) = param_name.strip_suffix('*') {
                ...
                left = new_left;
                let ext_value = super::extended::parse_extended_value(ext_value)?;
                ...
                let value = if left.starts_with('\"') {
                    ...
                    left = &left[end.ok_or(std::io::Error::new(
                        std::io::ErrorKind::InvalidInput,
                        "invalid parameter",
                    ))? + 1..];
                    left = split_once(left, ';').1.trim_start();
                    // In fact, it should not be Err if the above code is correct.
                    String::from_utf8(quoted_string).map_err(|_| {
                        std::io::Error::new(std::io::ErrorKind::InvalidInput, "invalid parameter")
                    })?
                } else {
...
impl Header for ContentDisposition {
        ...
        I: Iterator<Item = &'i HeaderValue>,
    {
        values
            .next()
            .and_then(|hv| Self::from_raw(hv).ok())
