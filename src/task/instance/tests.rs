...
use super::{TaskEvent, TaskInstanceImpl, ProgressWithSpeed, TaskInstance};
...
impl BoltLoadAdapter for MockAdapter {
    ...
    async fn retrieve_meta(&self) -> Result<BoltLoadAdapterMeta, UnretryableError> {
        if self.should_fail {
            return Err(UnretryableError::Internal("Mock adapter failure".to_string()));
        }
        Ok(self.meta.clone())
    }
...
async fn test_singleton_task_basic_download() {
    ...
    let adapter = Arc::new(Box::new(MockAdapter::new(content.clone())) as Box<dyn BoltLoadAdapter + Send>);
    ...
    let events = wait_for_completion(event_rx, Duration::from_secs(10)).await.unwrap();
...
async fn test_singleton_task_adapter_failure() {
    let rt = create_test_runtime();
    let adapter = Arc::new(Box::new(MockAdapter::new(vec![]).with_failure(true)) as Box<dyn BoltLoadAdapter + Send>);
    ...
    let task_failed = task_result.is_err() || 
        (events_result.is_ok() && events_result.unwrap().iter().any(|e| matches!(e, TaskEvent::Failed(_))));
...
async fn test_concurrent_vs_singleton_comparison() {
    ...
    let adapter_concurrent = Arc::new(Box::new(MockAdapter::new(content.clone())) as Box<dyn BoltLoadAdapter + Send>);
    ...
    // Test singleton task
    let adapter_singleton = Arc::new(Box::new(MockAdapter::new(content.clone()).with_range_support(false)) as Box<dyn BoltLoadAdapter + Send>);
    ...
    // Wait for task completion
    let (concurrent_wait, singleton_wait) = tokio::join!(
