...
const HTTP_VALUE: &AsciiSet = &percent_encoding::CONTROLS
    .add(b' ')
    .add(b'"')
    .add(b'%')
    .add(b'\'')
...
pub fn parse_extended_value(val: &str) -> std::io::Result<ExtendedValue> {
    // Break into three pieces separated by the single-quote character
    let mut parts = val.splitn(3, '\'');

    // Interpret the first piece as a Charset
    let charset: String = match parts.next() {
        None => {
...
mod tests {
    ...
    fn test_parse_extended_value_with_encoding_and_language_tag() {
        ...
        assert_eq!("iso-8859-1".to_owned(), extended_value.charset);
    ...
    fn test_parse_extended_value_with_encoding() {
        ...
        assert!(result.is_ok());
        let extended_value = result.unwrap();
        assert_eq!("UTF-8".to_string(), extended_value.charset);
